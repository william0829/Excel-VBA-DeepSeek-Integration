...
Private Const DEFAULT_MODEL As String = "deepseek-chat" ' AI模型，V3 = "deepseek-chat",R1 = "deepseek-reasoner"
Private Const DEFAULT_TEMPERATURE As Double = 0.7 ' Temperature创造力的温度，越大越开放 (0 to 2)
Private Const DEFAULT_MAX_TOKENS As Long = 300 ' 最大生成tokens数( 1 to 8192)
Private Const DEFAULT_TOP_P As Double = 1 ' top_p生成文本时的词汇选择范围 (0 to 1)
Private Const DEFAULT_FREQUENCY_PENALTY As Double = 0 
' 用于惩罚模型生成文本中已经出现过的词汇或内容。正数更倾向于生成新的，增加生成文本的多样性；负数则使模型更倾向于重复。 (-2 to 2)
Private Const DEFAULT_PRESENCE_PENALTY As Double = 0 
' 用于惩罚模型生成文本中某个词汇或内容的出现频率。与 PRESENCE_PENALTY 类似，正数避免过度重复；负数更频繁地生成某些词汇或内容。 (-2 to 2)

' HTTP 对象配置
Private Const USE_SERVER_XMLHTTP As Boolean = False 
' Set to True to use MSXML2.ServerXMLHTTP (for server environments)
...
' frequency_penalty”：可选参数，用于惩罚重复的标记，取值范围是 -2 到 2，默认值是 0。
' presence_penalty”：可选参数，用于鼓励新的主题，取值范围是 -2 到 2，默认值是 0。
' ========================================================
...
' 使用名称获取 API 密钥：
'     在工作簿中创建名为 “DS_API_KEY” 的名称，并在相应单元格中输入 API 密钥。
...
Function DS_Chat(prompt As String, Optional model As String = DEFAULT_MODEL, _
    ...
    ' Get API key from named range or use the constant 从名称获取 API 密钥或使用常量。
